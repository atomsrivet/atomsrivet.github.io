...
function runExperiment(){
  ...
  if (board.paper.centre[0]>board.paper.centre[1]) { //assume that if wider than longer, then computer
    scale = board.paper.centre[1]/9/50;}
  else {
    scale = board.paper.centre[0]/9/50; //assume that if longer than wider, then mobile
    if (board.paper.centre[1]<(8*scale*50)){
  scale = board.paper.centre[1]/9/50;
    }}
  // fonts
...
var rect = [x-scale*200,y-scale*90,scale*400,scale*180];
  ...
  if (mobile==0) {
    ...
    //board.dm.object.attr({"text-anchor": "centre"});
      ...
      board.dr.centre = [board.paper.centre[0]+scale*width*Math.ceil(coding.numrows/2), board.paper.centre[1]-scale*width*Math.ceil(coding.numrows/2)- 0.5*(1.63*1.5*width) - 0.5*board.font_medsize];
      board.dl.centre = [board.paper.centre[0]-scale*width*Math.ceil(coding.numrows/2), board.paper.centre[1]-scale*width*Math.ceil(coding.numrows/2)-0.5*(1.63*1.5*width) - 0.5*board.font_medsize];
